...
if side_option=='Data Sample':
    ...
    if how_pred == 'Number':
        ...
        with st.sidebar:
            ...
            upper_lim = st.number_input(label='Enter upper limit', min_value=-df[3].shape[0], max_value=df[3].shape[0], value=df[3].shape[0], key='for range upper limit')
...
@st.cache
def get_data():
    test_data_ben = pd.read_csv('D:/ML Course/Case Study 1/archive/Test_Beneficiarydata-1542969243754.csv')
    test_data_inp = pd.read_csv('D:/ML Course/Case Study 1/archive/Test_Inpatientdata-1542969243754.csv')
    test_data_out = pd.read_csv('D:/ML Course/Case Study 1/archive/Test_Outpatientdata-1542969243754.csv')
    test_ddata_merged = preparing_data(test_data_ben, test_data_inp, test_data_out)
    return (test_data_ben, test_data_inp, test_data_out, test_ddata_merged)
